...
int main(int argc, char* argv[]) {
	...
	const auto isotherm_eos = [&](const std::array<double, 86> &Y_, const double T, const double rho_) {
	...
	for (int i = 1; i <= n_max; ++i) {
		...
		auto [Y, T, current_dt] = isotherm ? 
			solve_system_NR(nnet::net86::reaction_list, nnet::net86::compute_reaction_rates<double>, nnet::net86::compute_BE<double>, isotherm_eos,
				last_Y, last_T, rho, 0., dt) :
			solve_system_NR(nnet::net86::reaction_list, nnet::net86::compute_reaction_rates<double>, nnet::net86::compute_BE<double>, helm_eos,
				last_Y, last_T, rho, 0., dt);
		t += current_dt;
